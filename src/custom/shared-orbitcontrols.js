...
import {ThreeLayer} from '../maptalks.three/index';
...
export function init(data) {   /* eslint-disable-line no-unused-vars */
  ...
  const {canvas, inputElement, mapConfig, browser, style} = data;
  Object.assign(Browser, browser);
  Object.assign(_style, style);
  console.log(Browser);
  console.log(_style);

  Object.assign(inputElement, mapConfig);
  //config(mapConfig);
  ...
  let maptalks = new Map(renderer.getContext(), inputElement, mapConfig);
  let threeLayer = new ThreeLayer('t');
  threeLayer.context = renderer;
  threeLayer.gl = renderer.getContext();
  threeLayer.canvas = renderer.getContext().canvas;
  //threeLayer.scene = scene;
  
  threeLayer.prepareToDraw = function (gl, scene2, camera) {
