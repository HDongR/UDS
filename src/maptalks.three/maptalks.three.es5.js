...
(function (global, factory) {
  ...
  var REVISION = parseInt(THREE.REVISION.replace('dev', '')); //Three does not print version information now. Output the version of three to find compatibility problems
  ...
  var LineSegmentsGeometry = /*#__PURE__*/function (_THREE$InstancedBuffe) {
    ...
    function LineSegmentsGeometry() {
      ...
      addAttribute(_assertThisInitialized(_this), 'position', new THREE.Float32BufferAttribute(positions, 3));
      addAttribute(_assertThisInitialized(_this), 'uv', new THREE.Float32BufferAttribute(uvs, 2));
      return _this;
    ...
    _proto.toJSON = function toJSON() {// todo
    } // clone: function () {
    //     // todo
    // },
  ...
  var LineSegments2 = /*#__PURE__*/function (_THREE$Mesh) {
    ...
    function LineSegments2(geometry, material) {
      ...
      _this.material = material !== undefined ? material : new LineMaterial({
  ...
  function generateBufferGeometry(data) {
    ...
    var bufferGeomertry = new THREE.BufferGeometry(); // addAttribute(bufferGeomertry, 'color', new THREE.BufferAttribute(color, 3));
  ...
  var topColor = new THREE.Color('#fff'),
      bottomColor = new THREE.Color('#fff');
  ...
  function initVertexColors(geometry, color, _topColor, key, v) {
    ...
    for (var i = 0; i < len; i += 3) {
      ...
      if (y > v) {
        ...
        colors[i + 2] = topColor.b; // colors.push(topColor.r, topColor.g, topColor.b);
      } else {
        colors[i] = bottomColor.r;
        colors[i + 1] = bottomColor.g;
        colors[i + 2] = bottomColor.b; // colors.push(bottomColor.r, bottomColor.g, bottomColor.b);
      }
    }
  ...
  function mergeBarGeometry(geometries) {
    ...
    if (colors.length) {
      addAttribute(bufferGeometry, 'color', new THREE.BufferAttribute(colors, 3, true)); // for (let i = 0, len = colors.length; i < len; i++) {
  ...
  */
  ...
  function getLinePosition(lineString, layer, center) {
    ...
    var positions = new Float32Array(positionsV.length * 3);
    var positions2d = new Float32Array(positionsV.length * 2);
  ...
  function getExtrudeLineGeometry(lineString, lineWidth, depth, layer, center) {
    ...
    var _getExtrudeLineParams = getExtrudeLineParams(lineString, lineWidth, depth, layer, center),
  ...
  var topColor$1 = new THREE.Color('#fff'),
      bottomColor$1 = new THREE.Color('#fff');
  /**
  ...
  function initVertexColors$1(geometry, color, _topColor, minZ) {
    ...
    if (Array.isArray(minZ)) {
      ...
      for (var _i2 = 0; _i2 < len; _i2 += 3) {
        ...
        if (_z > minZ) {
          colors[_i2] = topColor$1.r;
          colors[_i2 + 1] = topColor$1.g;
          colors[_i2 + 2] = topColor$1.b;
        } else {
          colors[_i2] = bottomColor$1.r;
          colors[_i2 + 1] = bottomColor$1.g;
          colors[_i2 + 2] = bottomColor$1.b;
        }
      }
    }
  ...
  function getPolygonPositions(polygon, layer, center, centerPt, isArrayBuff) {
    ...
    if (polygon instanceof maptalks.MultiPolygon) {
      ...
      var data = getSinglePolygonPositions(polygon, layer, center || polygon.getCenter(), centerPt, isArrayBuff);
      datas.push(data);
    } else if (isGeoJSONPolygon(polygon)) {
      // const cent = getGeoJSONCenter(polygon);
      if (!isGeoJSONMulti(polygon)) {
        var _data = getSinglePolygonPositions(polygon, layer, center || getGeoJSONCenter(polygon), centerPt, isArrayBuff);
  ...
  function getSinglePolygonPositions(polygon, layer, center, centerPt, isArrayBuff) {
    if (isArrayBuff === void 0) {
      isArrayBuff = false;
    }
    ...
    if (holes && holes.length > 0) {
      for (var _i3 = 0, _len3 = holes.length; _i3 < _len3; _i3++) {
        var pts = isArrayBuff ? new Float32Array(holes[_i3].length * 2) : [];

        for (var j = 0, len1 = holes[_i3].length; j < len1; j++) {
          var _c = holes[_i3][j];
  ...
  var ExtrudeLine = /*#__PURE__*/function (_BaseObject) {
    ...
    function ExtrudeLine(lineString, options, material, layer) {
      ...
      for (var i = 0, len = lineStrings.length; i < len; i++) {
        var attribute = getExtrudeLineParams(lineStrings[i], w, h, layer, center);
  ...
  var MergedMixin = function MergedMixin(Base) {
    return /*#__PURE__*/function (_Base) {
      ...
      _proto._setPickObject3d = function _setPickObject3d() {
        ...
        addAttribute(geometry, 'color', new THREE.BufferAttribute(colors, 3, true)); // const material = new THREE.MeshBasicMaterial();
  ...
  function gengerateExtrudePolygons(polygons, center, layer) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var data = getPolygonPositions(polygon, layer, center, centerPt, true);
      ...
      var properties = isGeoJSONPolygon(polygon) ? polygon['properties'] : polygon.getProperties() || {};
  ...
  function gengerateExtrudeLines(lineStringList, center, layer, lineStrings) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      for (var j = 0, len1 = multiLineString.length; j < len1; j++) {
        ...
        var positions2d = getLinePosition(lineString, layer, center).positions2d;
  ...
  var TEMP_COORD = new maptalks.Coordinate(0, 0);
  ...
  var ExtrudePolygons = /*#__PURE__*/function (_MergedMixin) {
    ...
    function ExtrudePolygons(polygons, options, material, layer) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        var _center = polygon.getCenter ? polygon.getCenter() : getGeoJSONCenter(polygon, TEMP_COORD);
      ...
      if (asynchronous) {
        ...
        actor.pushQueue({
          ...
          callback: function callback(e) {
            ...
            _this._geometryCache = generatePickBufferGeometry(bufferGeometry);
        ...
        for (var _i = 0; _i < len; _i++) {
          ...
          var properties = isGeoJSONPolygon(_polygon) ? _polygon['properties'] : _polygon.getProperties() || {};
          var height = properties.height || 1;
          var bottomHeight = properties.bottomHeight || 0;
          var buffGeom = getExtrudeGeometryParams(_polygon, height, layer, center, centerPt, altCache);
  ...
  var Points = /*#__PURE__*/function (_MergedMixin) {
    ...
    function Points(points, options, material, layer) {
      ...
      var hasColor = false,
          hasSize = false;
      var TEMP_VECTOR = new THREE.Vector3(0, 0, 0);
    ...
    _proto.getSelectMesh = function getSelectMesh() {
      ...
      if (index != null) {
        if (!this._baseObjects[index]) {
          ...
          var opts = Object.assign({}, this.options, isGeoJSONLine(lineString) ? lineString.properties : lineString.getProperties(), {
            index: index
          });
  ...
  var Lines = /*#__PURE__*/function (_MergedMixin) {
    ...
    function Lines(lineStrings, options, material, layer) {
      ...
      for (var _i = 0; _i < len; _i++) {
        ...
        for (var m = 0, le = lls.length; m < le; m++) {
          var properties = isGeoJSONLine(lls[m]) ? lls[m]['properties'] : lls[m].getProperties() || {};
  ...
  */
  ...
  function getPlaneGeometry(width, height, devideW, devideH) {
    var _getPlaneGeometryAttr = getPlaneGeometryAttribute(width, height, devideW, devideH),
  ...
  var Terrain = /*#__PURE__*/function (_BaseObject) {
    ...
    function Terrain(extent, options, material, layer) {
      ...
      var rgbImg = generateImage$1(image),
          img = generateImage$1(texture); // const geometry = new THREE.PlaneBufferGeometry(w, h, imageWidth - 1, imageHeight - 1);
  ...
  var HeatMap = /*#__PURE__*/function (_BaseObject) {
    ...
    function HeatMap(data, options, material, layer) {
      ...
      var geometry = getPlaneGeometry(offsetX, offsetY, canvasWidth - 1, canvasHeight - 1);
  ...
  var Box = /*#__PURE__*/function (_BaseObject) {
    ...
    function Box(coordinate, options, material, layer) {
      ...
      options = maptalks.Util.extend({}, OPTIONS$j, options, {
        layer: layer,
        coordinate: coordinate
      });
      _this = _BaseObject.call(this) || this;
  ...
  var TEMP_COORD$1 = new maptalks.Coordinate(0, 0);
  var TEMP_POINT = new maptalks.Point(0, 0); // const MATRIX4 = new THREE.Matrix4();
  ...
  var ThreeLayer = /*#__PURE__*/function (_maptalks$CanvasLayer) {
    ...
    _proto.coordinateToVector3 = function coordinateToVector3(coordinate, z) {
      ...
      if (isArray) {
        TEMP_COORD$1.x = coordinate[0];
        TEMP_COORD$1.y = coordinate[1];
      }
      ...
      var p = coordinateToPoint(map, isArray ? TEMP_COORD$1 : coordinate, res, TEMP_POINT);
  ...
  var ThreeRenderer = /*#__PURE__*/function (_maptalks$renderer$Ca) {
    ...
    _proto2.renderScene = function renderScene() {
      var time = maptalks.Util.now(); // Make sure to execute only once in a frame
  ...
  typeof console !== 'undefined' && console.log('maptalks.three v0.17.2');
