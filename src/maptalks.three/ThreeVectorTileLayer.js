...
class ThreeVectorTileLayer extends BaseVectorTileLayer {
    constructor(url, options = {}, getMaterial, layer) {
        super(Util.GUID(), Util.extend({ urlTemplate: url }, OPTIONS, options));
        this._opts = options;
        this._layer = layer;
            ...
            if (features && features.length) {
                ...
                for (let i = 0, len = features.length; i < len; i++) {
                    ...
                    if (isGeoJSONPolygon(feature)) {
                        ...
                        const fs = spliteGeoJSONMulti(feature);
                        for (let j = 0, len1 = fs.length; j < len1; j++) {
                            points.push(Util.extend({}, fs[j].properties, fs[j], { coordinate: getGeoJSONCoordinates(fs[j]) }));
                        }
                    }
                }
                if (polygons.length) {
                    ...
                    if (material) {
                        const extrudepolygons = this._layer.toExtrudePolygons(polygons, Util.extend({}, { topColor: '#fff', layerName, asynchronous, key: index }, opts), material);
                ...
                if (lineStrings.length) {
                    ...
                    if (material && (material instanceof THREE.LineBasicMaterial || material instanceof THREE.LineDashedMaterial)) {
                        const lines = this._layer.toLines(lineStrings, Util.extend({}, { layerName }, opts), material);
                ...
                if (points.length) {
                    ...
                    if (material && material instanceof THREE.PointsMaterial) {
                        const ps = this._layer.toPoints(points, Util.extend({}, { layerName }, opts), material);
                        baseobjects.push(ps);
                    }
                }
            }
